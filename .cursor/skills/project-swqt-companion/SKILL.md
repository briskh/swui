...
- External production identity observed 2026-07-24: anonymous `https://agent.swqt.net/mcp/swui` remains **swui 1.0.0**, `resources/list`=10, `templates/list`=1, `tools/list`=4; Button payload has no `demoUrl`, `contractRefs`, or `referenceSite`. npm `@swqt/ui` and `@swqt/ui-tokens` remain **1.0.0**. `https://ui.swqt.net` returns HTTP 200, but `/icons` has no Icons heading and `/agent` has no foundation-contract entry. These are explicit stale/pending distribution facts until separately authorized publish/deploy; local gates do not imply remote freshness.
